...
| --- | --- | --- |
| `HARBOR_ADDR` | `:7750` | Server listening address and port |
...
| `HARBOR_PORT` | `7750` | Host port binding in the supplied Compose file only |
...
`HARBOR_PORT=127.0.0.1:7750`; use an appropriate reachable binding for NAS clients.
...
mapping. Keep the container listener on port 7750 unless you also update port
